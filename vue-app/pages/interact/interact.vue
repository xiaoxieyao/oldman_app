...
// 通用点击动画函数 - 项目统一逻辑，和首页/社区活动页完全一致
const handleFunctionClick = (callback) => {
  isClick.value = true;
  setTimeout(() => {
    isClick.value = false;
    callback(); // 执行传入的点击逻辑（跳转/功能）
  }, 100);
};

// 社区活动 - 跳转到社区活动列表页（page-community）
const showMoreActivity = () => {
  // 复用项目统一的点击动画逻辑，和其他按钮交互一致
  handleFunctionClick(() => {
    console.log('执行社区活动页跳转，路径：/pages/community/community');
    uni.navigateTo({
      // 核心：填写社区活动页的实际路径（需和pages.json中注册的路径完全一致）
      url: '/pages/community/community',
      // 跳转失败的异常处理，排查问题更方便
      fail: (err) => {
        console.error('社区活动页跳转失败：', err);
        uni.showToast({
