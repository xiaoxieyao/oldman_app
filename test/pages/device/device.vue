...
// 健康数据跳转 - 改为实际跳转（匹配pages.json路径：pages/health/health）
const gotoHealth = () => {
  handleFunctionClick(() => {
    console.log('执行健康数据页跳转，路径：/pages/health/health');
    uni.navigateTo({
      url: '/pages/health/health',
      fail: (err) => {
        console.error('健康数据页跳转失败：', err);
        uni.showToast({
          title: `跳转失败：${err.errMsg || '页面不存在'}`,
          icon: 'none',
          duration: 3000
...
// 亲情互动跳转 - 改为实际跳转（匹配pages.json路径：pages/interact/interact）
const gotoInteract = () => {
  handleFunctionClick(() => {
    console.log('执行亲情互动页跳转，路径：/pages/interact/interact');
    uni.navigateTo({
      url: '/pages/interact/interact',
      fail: (err) => {
        console.error('亲情互动页跳转失败：', err);
        uni.showToast({
          title: `跳转失败：${err.errMsg || '页面不存在'}`,
          icon: 'none',
          duration: 3000
...
// 设备管理跳转 - 原有正确代码（保留，无需修改）
const gotoDeviceManage = () => {
  handleFunctionClick(() => {
    console.log('执行设备管理页跳转，路径：/pages/device-manage/device-manage');
    uni.navigateTo({
      url: '/pages/device-manage/device-manage',
      fail: (err) => {
        console.error('设备管理页跳转失败：', err);
        uni.showToast({
          title: `跳转失败：${err.errMsg || '页面不存在'}`,
          icon: 'none',
          duration: 3000
